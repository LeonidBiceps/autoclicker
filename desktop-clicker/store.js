...
const DEFAULT_SETTINGS = {
  ...
  clashTracker: {
    ...
    enabled: false,
    region: null, // { x, y, width, height } — легаси: абсолютные координаты, если привязка к окну не настроена
    windowTitle: null, // часть заголовка окна эмулятора — если задано, region ниже пересчитывается от ТЕКУЩЕГО положения этого окна на экране (не ломается при перемещении/пересоздании окна)
    regionOffset: null, // { x, y, width, height } — смещение области поля от левого верхнего угла окна эмулятора
    confidence: 0.85,
    ...
    matchActiveConfidence: 0.8, // с запасом ниже строгого 0.85 — видео-рендер эмулятора может слегка отличаться кадр от кадра даже для статичной иконки
  },
};
